...
void parseAndActCommand(void* para)
{
	FileInfo *pFileInfo =(FileInfo*)para;
	int i;
	char a[MAX_SIZE][MAX_SIZE]={0};
	if(pFileInfo == NULL || pFileInfo->command == NULL)
	{
		return;
	}
	//parse command string
	str_depart(pFileInfo->command,a);
...
void main()
{
	...
	{
		printMessage(0);
		//memset(fileInfo.command,0,sizeof(fileInfo.command));
		ret=scanf("%s",fileInfo.command);
		if(ret<=0)
		{
			printf("command %s is invalid",fileInfo.command);
			printf(",please input again!\n");
			continue;
		}
		if(!strcmp(fileInfo.command,"quit") || !strcmp(fileInfo.command,"QUIT"))
		{
			break;
		}
		pthread_create(&id,NULL,
			(void*)parseAndActCommand,&fileInfo);
	}
	//bye gui
	welcomeAndBye(1);
}
